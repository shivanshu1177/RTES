...
public:
    ...
    auto map(F&& func) -> Result<decltype(func(std::declval<T>()))> {
        using R = Result<decltype(func(std::declval<T>()))>;
        if (has_value_) {
            return R(func(value_));
        }
        return R(error_);
    }
...
public:
    NetworkErrorRecovery(int max_retries = 3,
    ...
    void set_connection_factory(std::function<Result<void>()> factory) {
