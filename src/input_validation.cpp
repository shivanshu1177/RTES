...
namespace rtes {
std::string FieldValidators::sanitize_string(const std::string& input, size_t max_length) {
    std::string result;
...
namespace rtes {
ValidationRule FieldValidators::alphanumeric_validator() {
    return ValidationRule(
        [](const std::string& value) {
...
bool FieldValidators::validate_price_quantity_relationship(Price price, Quantity quantity) {
    return quantity > 0; // price=0 is valid for market orders
}
} // namespace rtes
