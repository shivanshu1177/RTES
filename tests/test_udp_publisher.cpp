...
// FIX: REMOVED #include "rtes/market_data.hpp"
// market_data.hpp redefines UdpMessageType, UdpMessageHeader, BBOUpdateMessage,
// TradeUpdateMessage, and DepthLevel which are already defined in
// udp_publisher.hpp and order_book.hpp.
// All types needed by this test are already provided by udp_publisher.hpp.
...
TEST_F(UdpPublisherTest, HighThroughput) {
    ...
    // Wait for processing
    while (publisher->messages_sent() < static_cast<size_t>(num_events)) {
    ...
    EXPECT_EQ(publisher->messages_sent(), static_cast<size_t>(num_events));
    // FIX: bytes_sent() is not a public method. Use messages_sent() instead.
    // The line above already verifies messages were sent successfully.
    EXPECT_GT(publisher->messages_sent(), 0u);
}
