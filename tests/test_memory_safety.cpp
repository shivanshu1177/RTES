...
// FIX 1: UniqueBuffer was renamed to AlignedBuffer
// FIX 2: operator[] is unchecked (hot path); use .at() for bounds checking
TEST_F(MemorySafetyTest, AlignedBufferOperations) {
    // Test allocation and access
    AlignedBuffer<int> buffer(100);
    EXPECT_EQ(buffer.size(), 100);
    EXPECT_NE(buffer.get(), nullptr);
    
    // Test element access (unchecked operator[] for valid indices)
    buffer[0] = 42;
    buffer[99] = 84;
    EXPECT_EQ(buffer[0], 42);
    EXPECT_EQ(buffer[99], 84);
    
    // Test bounds checking (use .at() which throws on out-of-range)
    EXPECT_THROW(buffer.at(100), std::out_of_range);
    
    // Test move semantics
    AlignedBuffer<int> moved_buffer = std::move(buffer);
