...
TEST_F(MemoryPoolTest, OrderConstruction) {
    ...
    // Construct order in-place (use const char* instead of ClientID)
    new (order) Order(12345, "100", "AAPL", Side::BUY, OrderType::LIMIT, 1000, 15000);
    
    EXPECT_EQ(order->id, 12345);
    EXPECT_EQ(order->client_id, ClientID("100"));
    // Use .c_str() for FixedString comparison with STREQ
    EXPECT_STREQ(order->symbol.c_str(), "AAPL");
