...
protected:
    ...
    // Fixed: Order constructor expects const char*, not ClientID
    Order* create_order(OrderID id, const char* client_id, Side side, Quantity qty, Price price) {
...
TEST_F(MatchingEngineTest, OrderCancellation) {
    ...
    EXPECT_TRUE(engine->cancel_order(1, ClientID("100")));
    std::this_thread::sleep_for(std::chrono::milliseconds(5));
