...
// FIX: TradeCallback is now void(*)(const Trade&, void*)
// Capturing lambdas cannot convert, so use a free function + context pointer.
static void perf_trade_handler(const Trade& t, void* ctx) {
    static_cast<std::vector<Trade>*>(ctx)->push_back(t);
}
...
TEST_F(PerformanceRegressionTest, OrderBookAddOrderLatency) {
    ...
    auto latencies = measure_latencies_us([&, order_id = 0]() mutable {
        ...
        // FIX: Consume [[nodiscard]] return value
...
TEST_F(PerformanceRegressionTest, OrderBookMatchingLatency) {
    ...
    // FIX: Use function pointer + context instead of capturing lambda
    OrderBook book("AAPL", *pool, perf_trade_handler, &trades);
...
TEST_F(PerformanceRegressionTest, OrderBookDepthSnapshotLatency) {
    ...
    for (int i = 0; i < BOOK_SIZE; ++i) {
        ...
        // FIX: Consume [[nodiscard]] return value
        (void)book.add_order(buy_order);
        
        auto* sell_order = pool->allocate();
        sell_order->id = BOOK_SIZE + i + 1;
        ...
        // FIX: Consume [[nodiscard]] return value
        (void)book.add_order(sell_order);
    }
    
    // FIX: get_depth() now uses out-parameter instead of return value
    auto latency = measure_latency_us([&]() {
        DepthSnapshot depth;
        book.get_depth(depth, 10);
    }, ITERATIONS);
...
TEST_F(PerformanceRegressionTest, SPSCQueueLatency) {
    ...
    auto latency = measure_latency_us([&]() {
        // FIX: Consume [[nodiscard]] return values
        (void)queue.push(order);
        Order* popped;
        (void)queue.pop(popped);
    }, ITERATIONS);
