...
TEST_F(InputValidationTest, FieldValidators) {
    ...
    EXPECT_FALSE(sym_val.validator("MSFT123")); // digits not allowed by symbol_validator
...
TEST_F(InputValidationTest, InputSanitization) {
    // FIX: "test\x00\x01string" has exactly 12 bytes:
    //   t(1) e(2) s(3) t(4) \x00(5) \x01(6) s(7) t(8) r(9) i(10) n(11) g(12)
    // Original used length 15, which read 3 garbage bytes past the literal,
    // producing "teststring48" instead of "teststring".
    std::string input_with_control("test\x00\x01string", 12);
    std::string sanitized = InputSanitizer::sanitize_network_input(input_with_control);
    EXPECT_EQ(sanitized, "teststring");
...
TEST_F(InputValidationTest, CrossFieldValidation) {
    ...
    // Note: 70000 overflows uint16_t; max valid port is 65535 which passes range check
