...
TEST_F(IntegrationTest, EndToEndOrderFlow) {
    ...
    // Verify orders were processed (use get_stats() instead of direct methods)
    auto stats = risk_manager->get_stats();
    EXPECT_EQ(stats.processed, 2);
...
TEST_F(IntegrationTest, HighVolumeStressTest) {
    ...
    for (int i = 0; i < num_orders; ++i) {
        ...
        std::string client_str = std::to_string(100 + (i % 10));
        Side side = (i % 2 == 0) ? Side::BUY : Side::SELL;
        Price price = 15000 + (i % 20);  // Spread across price levels
        
        new (order) Order(i + 1, client_str.c_str(), "AAPL", side, OrderType::LIMIT, 100, price);
